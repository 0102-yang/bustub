...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower" && args.size() == 1) {
    return std::make_shared<StringExpression>(args.front(), StringExpressionType::Lower);
  }
  if (func_name == "upper" && args.size() == 1) {
    return std::make_shared<StringExpression>(args.front(), StringExpressionType::Upper);
  }
