 ...
 */
template <>
auto DiskExtendibleHashTable<int, int, IntComparator>::Hash(const int key) const -> uint32_t {
...
void DiskExtendibleHashTable<K, V, KC>::PrintHT() const {
  ...
  for (uint32_t idx = 0; idx < header->MaxSize(); idx++) {
    ...
    for (uint32_t idx2 = 0; idx2 < directory->Size(); idx2++) {
      const page_id_t bucket_page_id = directory->GetBucketPageId(idx2);
...
void DiskExtendibleHashTable<K, V, KC>::VerifyIntegrity() const {
  ...
  for (uint32_t idx = 0; idx < header->MaxSize(); idx++) {
    const auto directory_page_id = header->GetDirectoryPageId(idx);
