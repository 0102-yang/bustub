...
void InsertExecutor::Init() {
  LOG_TRACE("Insert executor Init");
...
auto InsertExecutor::Next(Tuple *tuple, [[maybe_unused]] RID *rid) -> bool {
  ...
  while (child_executor_->Next(&child_tuple, rid)) {
    LOG_DEBUG("Retrive tuple %s from child executor",
    ...
    if (const auto inserted_rid = table_info->table_->InsertTuple(inserted_tuple_meta, child_tuple); inserted_rid) {
      LOG_DEBUG("Insert new entry: RID %s, tuple %s", inserted_rid->ToString().c_str(),
                child_tuple.ToString(&child_executor_->GetOutputSchema()).c_str());
      // Insert indexes.
      for (const auto index_info : indexes_info) {
        ...
        LOG_DEBUG("Insert new index of RID %s to index %s", inserted_rid->ToString().c_str(),
  ...
  const Schema integer_schema({Column("inserted_rows", INTEGER)});
  *tuple = Tuple({Value{INTEGER, inserted_rows_count}}, &integer_schema);
  return true;
}
