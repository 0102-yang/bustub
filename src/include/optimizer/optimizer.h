...
class Optimizer {
 ...
 private:
   ...
   */
  auto OptimizeSeqScanAsIndexScan(const AbstractPlanNodeRef &plan) const -> AbstractPlanNodeRef;

  /** @brief check if the index can be matched */
  auto MatchIndex(const std::string &table_name,
                  uint32_t index_key_idx) const -> std::optional<std::tuple<index_oid_t, std::string>>;
