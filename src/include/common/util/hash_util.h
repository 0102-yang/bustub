...
class HashUtil {
 ...
 public:
  static auto HashBytes(const char *bytes, size_t length) -> hash_t {
  ...
  static auto CombineHashes(hash_t l, hash_t r) -> hash_t {
    hash_t both[2] = {};
    both[0] = l;
    both[1] = r;
  ...
  static auto SumHashes(hash_t l, hash_t r) -> hash_t { return (l % PRIME_FACTOR + r % PRIME_FACTOR) % PRIME_FACTOR; }

  template <typename T>
  static auto Hash(const T *ptr) -> hash_t {
    return HashBytes(reinterpret_cast<const char *>(ptr), sizeof(T));
  }

  template <typename T>
  static auto HashPtr(const T *ptr) -> hash_t {
    return HashBytes(reinterpret_cast<const char *>(&ptr), sizeof(void *));
  }

  /** @return the hash of the value */
  static auto HashValue(const Value *val) -> hash_t {
