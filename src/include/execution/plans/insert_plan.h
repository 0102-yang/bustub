...
class InsertPlanNode : public AbstractPlanNode {
 public:
   ...
   */
  InsertPlanNode(SchemaRef output, AbstractPlanNodeRef child, const table_oid_t table_oid)
      : AbstractPlanNode(std::move(output), {std::move(child)}), table_oid_(table_oid) {}

  /** @return The type of the plan node */
  [[nodiscard]] auto GetType() const -> PlanType override { return PlanType::Insert; }
