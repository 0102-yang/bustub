...
class TableHeap {
 ...
 public:
   ...
   * @param lock_mgr the lock manager that manages the locks
   * @param txn the transaction that inserts the tuple
   ...
   */
  void UpdateTupleMeta(const TupleMeta &meta, RID rid) const;
   ...
   */
  [[nodiscard]] auto GetTuple(RID rid) const -> std::pair<TupleMeta, Tuple>;
   ...
   * @return the meta
   */
  [[nodiscard]] auto GetTupleMeta(RID rid) const -> TupleMeta;
  ...
  [[nodiscard]] auto GetFirstPageId() const -> page_id_t { return first_page_id_; }
  ...
  [[nodiscard]] auto AcquireTablePageReadLock(RID rid) const -> ReadPageGuard;

  [[nodiscard]] auto AcquireTablePageWriteLock(RID rid) const -> WritePageGuard;
